...
export class SalesService {
  ...
  async create(createSaleDto: InvoiceDto) {
    ...
    const invoiceDetail = this.invoiceDetailRepository.create(createSaleDto.details.map(item => ({
      ...item,
  ...
  async findOne(id: number) {
    const invoice = await this.invoiceRepository.findOne({ where: { id }, relations: ['details'] })
    if (!invoice) throw new NotFoundException('The Invoice With id ' + id + ' Was Not Found')
    return invoice;
  }
  ...
  async update(id: number, updateSaleDto: UpdateInvoiceDto) {
    ...
    updateSaleDto.details.map(async item => (
      console.log(InvoiceDetail.create({
        ...item
      })),
      detailsToUdate.push(InvoiceDetail.create({
        ...item,
        invoiceId: id,
      }))
    ))
    ...
    let total = 0;
    const updated = await this.invoiceRepository.findOne({ where: { id }, relations: ['details'] });
  ...
  async remove(id: number) {
    const details = await this.invoiceDetailRepository.findBy({ invoiceId: id })
    details.map(async x => (
      await this.invoiceDetailRepository.delete(x.id)
    ))
